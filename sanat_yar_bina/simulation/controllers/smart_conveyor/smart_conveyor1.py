    ...
    while supervisor.step(TIME_STEP) != -1:
        ...
        if step_counter % 8 == 0:
            ...
            if camera_data:
                ...
                cv2.rectangle(annotated, (0, 0), (orig_width, 50), (0, 0, 0), -1)
                
                
               
                color_speed = (0, 255, 0) if current_speed > 0.25 else (0, 0, 255)
                cv2.putText(annotated, f"SPD:{current_speed*100:.0f}%", (5, 20), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, color_speed, 1)
                
                cv2.putText(annotated, f"RSK:{risk:.2f}", (130, 20), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
                
                fuz_text = "BRK" if fuzzy_suggestion < -0.1 else "ACC" if fuzzy_suggestion > 0.1 else "HLD"
                cv2.putText(annotated, f"Fuz:{fuz_text}", (5, 40), 
                ...
                cv2.putText(annotated, f"RL:{action_name}", (130, 40), 
